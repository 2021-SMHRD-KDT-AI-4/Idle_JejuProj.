					...
					<div class="box">
					 ...
					 // 마커 이미지를 생성 
						...
						function btn1(){
							...
							window.location.href='Choice?index=0&lat='+<%= recommend.get(0).getLat() %>+'&lon='+<%= recommend.get(0).getLon() %>;
						}
						function btn2(){
							alert('방문 후 사용자 게시판에 후기를 남겨주시면 응모권을 드립니다.!');
							window.location.href='Choice?index=1&lat='+<%= recommend.get(1).getLat() %>+'&lon='+<%= recommend.get(1).getLon() %>;
						}
						function btn3(){
							alert('방문 후 사용자 게시판에 후기를 남겨주시면 응모권을 드립니다.!');
...
<body class="is-preload">
		<div id="page-wrapper">
			...
			<!-- Main -->
				<section id="main" class="container">
					<header>
						<a href = "a_destination.jsp"><h2>DESTINATION</h2></a>
						<p>주변에 이런 관광지가 있네요~</p>
					</header>
					<div class="box">
						<span id = "map" class="image featured"><img src="images/pic01.jpg" alt="" /></span>
