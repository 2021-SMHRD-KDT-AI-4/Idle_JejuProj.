...
<body class="is-preload">
		<div id="page-wrapper">
			...
			<%
				...
				ArrayList<TourDTO> recommend_res= (ArrayList)session.getAttribute("recommend_res");
			...
			<!-- Main -->
				<section id="main" class="container">
					<header>
						<h2 class="jfont" style="font-size:100px;">RESTAURANT</h2>
						<p class="jfont" style="font-size:50px; padding:0px; margin:0px;">주변에 이런 음식점이 있네요~</p>
					</header>
					<div class="box">
						...
						var positions = [
						    {
						        title: '<%= recommend_res.get(0).getName() %>', 
						        latlng: new kakao.maps.LatLng(<%= recommend_res.get(0).getLat() %>,<%= recommend_res.get(0).getLon() %>)
						    },
						    {
						        title: '<%= recommend_res.get(1).getName() %>', 
						        latlng: new kakao.maps.LatLng(<%= recommend_res.get(1).getLat() %>, <%= recommend_res.get(1).getLon() %>)
						    },
						    {
						        title: '<%= recommend_res.get(2).getName() %>', 
						        latlng: new kakao.maps.LatLng(<%= recommend_res.get(2).getLat() %>, <%= recommend_res.get(2).getLon() %>)
						    }
						];
					 ...
					 // 마커 이미지를 생성 
						...
						function btn1(){
							...
							window.location.href='Choice?index=0&lat='+<%= recommend_res.get(0).getLat() %>+'&lon='+<%= recommend_res.get(0).getLon() %>;
						}
						function btn2(){
							alert('방문 후 사용자 게시판에 후기를 남겨주시면 응모권을 드립니다.!');
							window.location.href='Choice?index=1&lat='+<%= recommend_res.get(1).getLat() %>+'&lon='+<%= recommend_res.get(1).getLon() %>;
						}
						function btn3(){
							alert('방문 후 사용자 게시판에 후기를 남겨주시면 응모권을 드립니다.!');
						...
						<h3 class="jfont" style="font-size:50px;">내 주변 관광지 </h3>
							<table align = "center">
								<tr align = "center">
									<td stlye="width: 10px;">이름</td>
									<td><%= recommend_res.get(0).getName() %></td>
									<td><%= recommend_res.get(1).getName() %></td>
									<td><%= recommend_res.get(2).getName() %></td>
								</tr>
								<tr align = "center">
									<td stlye="width: 8px;"></td>
									<td><img src = "<%= recommend_res.get(0).getImg() %>"></td>
									<td><img src = "<%= recommend_res.get(1).getImg() %>"></td>
									<td><img src = "<%= recommend_res.get(2).getImg() %>"></td>
								</tr>
								<tr align = "center">
									<td stlye="width: 10px;">소개</td>
									<td><%= recommend_res.get(0).getInfo() %></td>
									<td><%= recommend_res.get(1).getInfo() %></td>
									<td><%= recommend_res.get(2).getInfo() %></td>
								</tr>
								<tr align = "center">
									<td stlye="width: 10px;">주소</td>
									<td><%= recommend_res.get(0).getAddr() %></td>
									<td><%= recommend_res.get(1).getAddr() %></td>
									<td><%= recommend_res.get(2).getAddr() %></td>
								</tr>
								<tr align = "center">
